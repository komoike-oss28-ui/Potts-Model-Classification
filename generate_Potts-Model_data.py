...
output_ml_folder = f'potts_q{q}_data_opt'
...
# 温度設定 (臨界点周辺)
beta_low = beta_c * 0.2
beta_high = beta_c * 1.8
beta_values = list(np.linspace(beta_low, beta_c * 0.8, 6)) + \
              list(np.linspace(beta_c * 1.2, beta_high, 6))
...
def generate_configs_fast(beta, q, num_configs, total_mcs, equilibration_mcs):
    ...
    print(f"  beta={beta:.4f} T={T:.4f} ({phase_label}, save every {save_interval} MCS)...", end="", flush=True)
    st = time.time()
    ...
    for mcs in range(1, total_mcs + 1):
        ...
        if mcs > equilibration_mcs:
            ...
            if steps_since_eq % save_interval == 0 and config_count < num_configs:
                fname = os.path.join(output_ml_folder, 
    ...
    print(f" Done: {config_count} configs saved ({elapsed:.2f}s, {mcs_per_sec:.1f} MCS/s)")
...
if __name__ == '__main__':
    ...
    for idx, beta in enumerate(beta_values):
        ...
        st = time.time()
        generate_configs_fast(beta, q, num_configs_per_temp, 
                            total_mcs_ml, equilibration_mcs_ml)
