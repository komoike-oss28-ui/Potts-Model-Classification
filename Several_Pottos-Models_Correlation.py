...
Q_PARAM = 3
TARGET_R = 32
MAP_TYPES_TO_RUN = ['fix_origin', 'spatial_avg', 'paper_style']
BC_SETTINGS = [False]
INPUT_FOLDER = f'potts_q{Q_PARAM}_train_data_opt'
...
def calculate_potts_correlation_ultra_opt(spins, r_vec_list, q, use_pbc, map_type, origin):
    """
    超最適化された相関計算
    - fix_originを完全ベクトル化
    - 不要な型変換を削減
...
if __name__ == '__main__':
    ...
    for map_type in MAP_TYPES_TO_RUN:
        for is_pbc in BC_SETTINGS:
            ...
            elapsed = time.time() - start_time
            print(f"   ✓ Completed in {elapsed:.2f}s ({len(input_files)/elapsed:.1f} files/s)")
